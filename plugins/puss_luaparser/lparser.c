...
static void recfield (LexState *ls, AstNodeList *fields) {
  ...
  if (ls->t.token == TK_NAME) {
    ...
    yindex(ls, &key);
    stat->field.k = key.expr;
  }
  checknext(ls, '=');
  expr(ls, &val);
  stat->field.v = val.expr;
}
...
static void listfield (LexState *ls, AstNodeList *fields) {
  ...
  expdesc v;
  expr(ls, &v);
  ast_nodelist_append(fields, v.expr);
}
...
static void constructor (LexState *ls, expdesc *t) {
  ...
  FuncState *fs = ls->fs;
  int line = ls->t.sline;
  AstNode *tb = ast_node_new(ls, AST_constructor, CTK, CTK);
  t->expr = tb;
  checknext(ls, '{');
  do {
    if (ls->t.token == '}') break;
    field(ls, &(tb->constructor.fields));
...
static int funcargs (LexState *ls, int line, AstNodeList *params) {
  FuncState *fs = ls->fs;
  switch (ls->t.token) {
    ...
    case '{': {  /* funcargs -> constructor */
      expdesc e;
      constructor(ls, &e);
      ast_nodelist_append(params, e.expr);
      break;
    }
...
static void primaryexp (LexState *ls, expdesc *v) {
  ...
  switch (ls->t.token) {
    ...
    case TK_NAME: {
      v->expr = ast_node_new(ls, AST_vname, CTK, CTK);
      luaX_next(ls);
      return;
    }
    default: {
  ...
  for (;;) {
    switch (ls->t.token) {
      ...
      case ':': {  /* ':' NAME funcargs */
		...
		exp->call.name->vname.parent = v->expr;
		v->expr = exp;
		if (check(ls, TK_NAME))
          luaX_next(ls);
        funcargs(ls, line, &(exp->call.params));
        break;
      }
...
static void simpleexp (LexState *ls, expdesc *v) {
  ...
  switch (ls->t.token) {
    case TK_FLT: {
      v->expr = ast_node_new(ls, AST_vflt, CTK, CTK);
      break;
    }
    case TK_INT: {
      v->expr = ast_node_new(ls, AST_vint, CTK, CTK);
      break;
    }
    case TK_STRING: {
      v->expr = ast_node_new(ls, AST_vstr, CTK, CTK);
      break;
    }
    case TK_NIL: {
      v->expr = ast_node_new(ls, AST_vnil, CTK, CTK);
      break;
    }
    case TK_TRUE: {
      v->expr = ast_node_new(ls, AST_vbool, CTK, CTK);
      break;
    }
    case TK_FALSE: {
      v->expr = ast_node_new(ls, AST_vbool, CTK, CTK);
    ...
    case TK_DOTS: {  /* vararg */
      v->expr = ast_node_new(ls, AST_vararg, CTK, CTK);
      break;
    }
    case '{': {  /* constructor */
      constructor(ls, v);
...
static BinOpr subexpr (LexState *ls, expdesc *v, int limit) {
  ...
  while (op != OPR_NOBINOPR && priority[op].left > limit) {
    AstNode *exp = ast_node_new(ls, AST_bin, v->expr->ts, CTK);
	expdesc v2;
    BinOpr nextop;
	exp->bin.op = CTK;
	exp->bin.l = v->expr;
...
static void forstat (LexState *ls, int line, const Token *tk) {
  ...
  if (!check(ls, TK_NAME)) {
    stats_append(ls, ast_error_newliteral(ls, "forstat need name", CTK, CTK));
    return;
  }
  var = ast_node_new(ls, AST_var, CTK, CTK);
  if (testnext(ls, ':')) {
  ...
  switch (ls->t.token) {
    ...
    case ',': case TK_IN:
      ...
      break;
    default:
      stats_append(ls, ast_error_newliteral(ls, "'=' or 'in' expected", CTK, CTK));
      return;
  }
  if (!testnext(ls, TK_END))
    stats_append(ls, ast_error_newliteral(ls, "match FOR END failed", CTK, CTK));
...
static int funcname (LexState *ls, expdesc *v, const Token *tk) {
  /* funcname -> NAME {fieldsel} [':' NAME] */
  int ismethod = 0;
  if (check(ls, TK_NAME)) {
    v->expr = ast_node_new(ls, AST_var, tk, CTK);
    luaX_next(ls);
  }
  else {
...
static void exprstat (LexState *ls) {
  ...
  if (ls->t.token == '=' || ls->t.token == ',') { /* stat -> assignment ? */
    ...
    if (v.v.expr && v.v.expr->type == AST_call) {
