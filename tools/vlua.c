// vlua.c : gcc -s -O2 -pthread -o vlua vlua.c -llua -lm -ldl
	...
	#define vlua_thread_detach	CloseHandle
#else
	#include <sys/time.h>
	#include <unistd.h>
	#include <limits.h>
	...
	#define vlua_thread_detach	pthread_detach
#endif
...
static VLuaThreadTask* vlua_thread_task_mq_pop(VLuaThreadMQ* mq) {
	VLuaThreadTask* res = NULL;
#ifdef _WIN32
	EnterCriticalSection(&mq->cs);
	...
	LeaveCriticalSection(&mq->cs);
	while( res==NULL ) {
		WaitForSingleObject(mq->ev, INFINITE);
		EnterCriticalSection(&mq->cs);
		res = __thread_task_mq_pop(mq);
		LeaveCriticalSection(&mq->cs);
	}
...
static int lua_thread_pool_destroy(lua_State* L) {
	...
	if( !ud->stop ) {
		...
		while( ud->remain > 0 ) {
			task = (VLuaThreadTask*)vlua_thread_task_mq_pop(&(ud->resp_mq));
			ud->remain--;
			if( task->req ) {
				free(task->pkt);
				free(task);
		...
		for( i=0; i<n; ++i ) {
			vlua_thread_detach(ud->threads[i].id);
...
static int lua_thread_pool_dispatch(lua_State* L) {
	VLuaThreadTask* task = lua_touserdata(L, 2);
	lua_settop(L, 1);
	vlua_pickle_unpack(L, task->key_buf, task->key_len);
	lua_pushboolean(L, task->resp_res==LUA_OK);
	if( task->resp_res )
...
static int lua_thread_pool_wait(lua_State* L) {
	...
	while( ud->remain ) {
		...
		lua_pushvalue(L, 2);
		lua_pushlightuserdata(L, vlua_thread_task_mq_pop(&(ud->resp_mq)));
		ud->remain--;
		if( vlua_pcall_stacktrace(L, 2, 0) ) {
			fprintf(stderr, "wait task error: %s\n", lua_tostring(L, -1));
		}
	}
	return 0;
}
...
static int run(lua_State* L, const char* script, int is_script_file) {
	...
	const char* main_thread_init_script =
		...
		"if n > 0 then vlua.thread_pool = vlua.thread_pool_create(n, vlua._script) end\n"
		"\n"
